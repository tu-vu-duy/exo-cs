...
public class MigrateService implements Startable{
  ...
  private void calendarMigration() {
    try{     
      ...
      String calendarData20 = params_.getValueParam("calendarData2.0.0-GA").getValue() ;
      NodeIterator iteratorUsers = getNodeByPath(nodeHierarchy_.getJcrPath(USERS_PATH), SessionProvider.createSystemProvider()).getNodes();
      while (iteratorUsers.hasNext()) {
        String userPath = iteratorUsers.nextNode().getPath(); 
        String username = userPath.substring(userPath.lastIndexOf("/") + 1);
      ...
      System.out.println("Colaboration Suite: Migrate Calendar data 1.3 to 2.0 is finished ..");
    }catch(Exception e) {
      e.printStackTrace() ;
    }
  }
  ...
  private void contactMigration() {
    try{     
      ...
      String contactData20 = params_.getValueParam("contactData2.0.0-GA").getValue() ;
      NodeIterator iteratorUsers = getNodeByPath(nodeHierarchy_.getJcrPath(USERS_PATH), SessionProvider.createSystemProvider()).getNodes();
      while (iteratorUsers.hasNext()) {
        String userPath = iteratorUsers.nextNode().getPath(); 
        String username = userPath.substring(userPath.lastIndexOf("/") + 1);
      ...
      System.out.println("Colaboration Suite: Migrate Contact data 1.3 to 2.0 is finished ");
    }catch(Exception e) {
      e.printStackTrace() ;
    }
  }
  ...
  private void mailMigration() {
    try{     
      ...
      String mailData20 = params_.getValueParam("mailData2.0.0-GA").getValue() ;
      NodeIterator iteratorUsers = getNodeByPath(nodeHierarchy_.getJcrPath(USERS_PATH), SessionProvider.createSystemProvider()).getNodes();
      while (iteratorUsers.hasNext()) {
        String userPath = iteratorUsers.nextNode().getPath(); 
        String username = userPath.substring(userPath.lastIndexOf("/") + 1);
      ...
      System.out.println("Colaboration Suite: Migrate Mail data 1.3 to 2.0 is finished ....");
    }catch(Exception e) {
      e.printStackTrace() ;
    }
  }
