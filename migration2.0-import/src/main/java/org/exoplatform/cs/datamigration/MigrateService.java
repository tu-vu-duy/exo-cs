...
public class MigrateService implements Startable{
  ...
  private void importNewCalendarData() {
    try{
      String file = params_.getValueParam("calendarData2.0.0-GA").getValue() ;
      System.out.println("\n Calendar's data version 2.0.0-GA is importing.... ");      
      ...
      System.out.println("\n >>>>>> Calendar's data version 2.0.0-GA is imported succesful !\n");      
    }catch(Exception e) {
      e.printStackTrace() ;
    }
  }
  ...
  private void importNewContactData() {
    try{
      String file = params_.getValueParam("contactData2.0.0-GA").getValue() ;
      System.out.println(" Contact's data version 2.0.0-GA is importing.... \n");
      NodeIterator iteratorUsers = getNodeByPath(nodeHierarchy_.getJcrPath(USERS_PATH), SessionProvider.createSystemProvider()).getNodes();
      while (iteratorUsers.hasNext()) {
        String userPath = iteratorUsers.nextNode().getPath(); 
        String username = userPath.substring(userPath.lastIndexOf("/") + 1);
      ...
      System.out.println("\n >>>>>> Contact's data version 2.0.0-GA is imported succesful !\n");      
    }catch(Exception e) {
      e.printStackTrace() ;
    }
  }
  ...
  private void importNewMailData() {
    try{
      String file = params_.getValueParam("mailData2.0.0-GA").getValue() ;
      System.out.println(" Mail's data version 2.0.0-GA is importing...\n");
      NodeIterator iteratorUsers = getNodeByPath(nodeHierarchy_.getJcrPath(USERS_PATH), SessionProvider.createSystemProvider()).getNodes();
      while (iteratorUsers.hasNext()) {
        String userPath = iteratorUsers.nextNode().getPath(); 
        String username = userPath.substring(userPath.lastIndexOf("/") + 1);
      ...
      System.out.println("\n >>>>>> Mail's data version 2.0.0-GA is imported succesful !\n");      
    }catch(Exception e) {
      e.printStackTrace() ;
    }
  }
